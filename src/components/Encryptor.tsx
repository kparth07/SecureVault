...
import { encryptFile } from '../crypto/engine';
import { formatBytes } from '../utils/format';

export default function Encryptor({ onBack }: { onBack: () => void }) {
  ...
  const handleEncrypt = async () => {
    ...
    try {
      const svaultBlob = await encryptFile(file, password, (p) => setProgress(p));
      const url = URL.createObjectURL(svaultBlob);
      setDownloadUrl(url);
    } catch (err) {
  ...
  // Determine actual status text based on state
  let statusText = "Waiting for file";
  if (file && !isProcessing && !downloadUrl) statusText = "Ready to encrypt";
  if (isProcessing) statusText = "Encrypting...";
  if (downloadUrl) statusText = "Encryption complete";
  if (error) statusText = "Encryption failed";
      ...
      <div className="bg-surface-container-lowest dark:bg-gray-900/50 rounded-3xl p-8 border border-outline-variant/30 dark:border-gray-700 shadow-xl shadow-indigo-500/5">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-headline-md font-headline-md text-on-surface dark:text-gray-100">Encrypt a File</h2>
        ...
        {!file ? (
          ...
          >
            <span className="material-symbols-outlined text-4xl text-outline mb-2 group-hover:text-primary dark:text-indigo-400 transition-colors">description</span>
            <p className="text-body-md text-on-surface dark:text-gray-100 font-medium">Drop your file here</p>
            <p className="text-body-sm text-on-surface-variant dark:text-gray-400 mt-1">or choose a file from your device</p>
          ...
          <div className="w-full rounded-2xl border border-primary/30 bg-primary/5 dark:bg-indigo-900/20 p-4 flex flex-col mb-8 gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 overflow-hidden">
                <span className="material-symbols-outlined text-primary dark:text-indigo-400" style={{ fontVariationSettings: "'FILL' 1" }}>draft</span>
                <div className="truncate">
                  <p className="text-body-md font-medium text-on-surface dark:text-gray-100 truncate">{file.name}</p>
                  <p className="text-label-sm text-on-surface-variant dark:text-gray-400">{formatBytes(file.size)} &bull; {file.type || 'Unknown type'}</p>
                </div>
              </div>
            ...
            {downloadUrl && (
              <div className="pt-4 border-t border-primary/20 flex flex-col gap-2">
                <div className="flex items-center gap-2 text-primary dark:text-indigo-400">
                  ...
                  <p className="text-body-md font-medium truncate">{file.name}.svault</p>
                </div>
                <div className="flex gap-2 text-label-sm text-on-surface-variant dark:text-gray-400">
                  ...
                  <span className="bg-surface-container-high dark:bg-gray-700 px-2 py-0.5 rounded text-xs font-mono">Client-side Encryption</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-2 mb-8">
          <label className="text-label-md text-on-surface-variant dark:text-gray-400">Encryption Password</label>
        ...
        {isProcessing ? (
          <div className="space-y-3">
            <div className="flex justify-between text-label-md">
              <span className="text-on-surface-variant dark:text-gray-400">Encrypting...</span>
          ...
          <div className="flex flex-col gap-4">
            <div className="p-4 rounded-xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 flex items-start gap-3 text-body-sm border border-emerald-500/20">
              ...
              <p>Successfully encrypted.</p>
            </div>
            <a 
              href={downloadUrl}
              download={`${file?.name}.svault`}
